...
- 主要交付：Crossref API adapter（DOI 存在性/元数据/更正关系；**撤稿/更正的单篇判定读 works JSON `updated-by[]`**——「本文被谁更新」，被撤稿论文在此字段含 `type=retraction`；**`update-to[]` 表示「本文更新了谁」，即本文是撤稿/更正声明本身，不得据它把本文判为被撤稿**；`filter=update-type:retraction` 用于**列表发现**撤稿记录，与单篇判定语义不同。Retraction Watch 2025-01 起并入其中。**字段方向依据 Crossref works JSON 语义，2026-09-11 经真实 API 复核**（PR #16 审查，见 B5 账本 Decisions）；PDF 解析层（**docling 默认，MIT，ADR-01 槽位 3**：模型权重离线预置后离线运行；pymupdf4llm 仅作 docling 权重不可用时的轻量兜底，AGPL 条款见 ADR-01 签字块 2，启用须在 receipt/PROGRESS 记录**原因**；解析产物进 B 线 candidate 通道，**无真实定位信息时 locator 留空以触发准入 fail-closed，不得以占位字符串充当 locator**）；评测用自建 gold set 构建（ScholarQABench 指标口径：citation recall / precision / hallucination ratio，供 A5 固定语料与主指标；**hallucination ratio 只计确定性不存在（`not_found`）；传输失败（`unknown`）单列 `undetermined_ratio`，不得并入主指标**，否则网络抖动会直接移动 A5 的裁决依据）。
