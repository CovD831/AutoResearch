...
| 2026-09-11 | **accepted (merged)** | PR #17 merged into main (`7fdfb89`); PR #16 closed. | `reviews/PR16-B5-*.md` | — |
| 2026-09-11 | **docling path verified** | Installed docling 2.126.0 + torch 2.14.0; two real arXiv PDFs parsed (`ok`, 49,240 / 70,915 chars, real locators), reproduced offline (`HF_HUB_OFFLINE=1`), and admitted E2E. Requirement: `HF_HOME` at a writable path; ~506MB weights pre-provisioned. | `/tmp/b5_acceptance.py`; `tests/test_b5_parse_acceptance_tmp.py` (2 passed, 81s; throwaway) | — |
