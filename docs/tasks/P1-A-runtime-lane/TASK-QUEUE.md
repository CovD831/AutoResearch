...
| A8 S4 主链路检索 adapter（2026-09-14，执行 `D-A5-偏离-1` 选项 (a)） | reviewing（PR #23） | A5 integrated（已满足） | 主链路从裸 `httpx.get` 匿名 connector 切到 A5 真实 adapter；新增 `retrieve()` 原语与 `AdapterBackedPaperSearchService` |
| A9 S4 交接单只传引用（2026-09-14，真实运行暴露的契约上限缺陷） | reviewing（PR #23） | A8（已满足） | `HandoffEnvelope` 由内容承载改为引用界；`paper_reader.py` 不再逐张铺卡；`paper_search.py` 的静默 `[:20]` 截断加可观测告警 |
| A10 S4 书目声称与部分失败（2026-09-14，第二轮独立盲审的 F4 + N1） | reviewing（PR #23） | A9（已满足） | 共享书目写入函数（parity 结构化）；claim 只描述实际材料；不可归属的检索命中拒绝铸 E1；`receipt.provider_failure` 让部分失败可见 |
