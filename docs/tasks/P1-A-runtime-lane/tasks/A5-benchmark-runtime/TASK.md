...
状态：`reviewing`（O7 S3 promotion 已于 2026-09-10 完成，A4 registry 已 accepted；PR #18 停在 handoff，owner 代修见 `owner/s4-a-integration`）。完整目标、交付物、边界和验收见 `../../TASK-SPECS.md` 的 A5 节；`task-package.json` 由成员实现时按实际命名填写（参照 A4 先例）。
...
- receipt 成本字段 schema 预留：**已交付**（`TokenUsageSlot` / `InvocationCostSlot`，字段逐一对位 O12 的 `contracts.TokenUsage` / `contracts.InvocationCost`；默认 `None` = 未测得，不得读成零成本）。端到端计价仍待 O12 交付后补验，不阻塞本包 accepted。
