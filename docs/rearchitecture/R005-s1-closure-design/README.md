...
本项目的目标是**整个 AutoResearch 项目的架构重构**。全项目的目标架构、阶段路线和 S1–S4 计划由 [R-004-TRUST-PROGRAM](../R004-trust-program/) 负责；本目录的 R005 只是当前首先落地的 **S1 Paper Search 纵向切片**，不是“项目只重构论文搜索”。

管理层先读：[EXECUTIVE-SUMMARY.md](EXECUTIVE-SUMMARY.md)。团队成员先读：[TEAM-READING-GUIDE.md](TEAM-READING-GUIDE.md)，讨论决策见 [DECISION-BOARD.md](DECISION-BOARD.md)。视觉版架构说明：[architecture-eli5-v3.html](architecture-eli5-v3.html)。工程架构图：[architecture-target.svg](architecture-target.svg)。
...
状态：design，checker 已通过；两轮审查均已登记并消费，P1 共同决策已按 [UD-006](decisions/UD-006-p1-freeze.json) 冻结。下一步是 `P1-IMPLEMENTATION-PACKAGE`：各任务负责人提交 task-local L3、fixtures、acceptance 和 rollback 后，再进入运行时代码实现。
