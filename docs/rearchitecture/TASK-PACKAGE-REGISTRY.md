...
| S4-A5-HANDOFF-BY-REFERENCE | S4 | reserved | reserved | planned（交接单由内容承载改为引用名单；`HandoffEnvelope` 上限重定义 + `paper_reader.py` 不再逐张铺卡 + `paper_search.py` 静默 `[:20]` 截断加告警；依赖 A8 先拿到真实量级） | A8 | A8 | — | — |
