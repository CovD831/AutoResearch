...
| S2-B-AUDIT-EVIDENCE | S2 | evidence/domain | `codex/s2-audit-evidence`（PR #9 open，97cd226） | reviewing（owner 判定 blocked：与 S2-A 文件/record kind 撞车 + base 落后；等布局裁决后 rebase。账本由 owner 按 D-SYNC-01 补建） | `main@6b706df`（实际 base `main@dad4658`） | S1 promotion | S2-A 布局裁决 | S3-B |
