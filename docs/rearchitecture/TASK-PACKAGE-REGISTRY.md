...
| S4-A5-HANDOFF-BY-REFERENCE | S4 | integration/lead（Owner 亲自实施，2026-09-14：改写冻结的 `contracts.py` HandoffEnvelope 语义，成员包不得单方面改写） | `owner/a8-mainline-adapter`（与 A8 同分支，未推送） | active（本地：交接单由内容承载改为引用界；基线 A8 525 → **532 passed**、判别力 4 判据型） | `owner/a8-mainline-adapter`（A8） | S4-A4-MAINLINE-ADAPTER（已满足） | — | — |
