"""S2-B Audit Evidence module (owner integration record, D-S2-01).

Implemented in B3 (member B, PR #9) as ``autoresearch/audit.py``; renamed to
``audit_evidence`` at owner integration so the A3 runtime boundary keeps the
canonical ``autoresearch.audit`` module. Record kind is ``audit_evidence_report``
and events use the ``audit_evidence.*`` namespace, separating them from the
A3 runtime's ``audit_report`` kind and ``audit.*`` events.
"""
...
class AuditService:
    ...
    @classmethod
    def _claim_matches_text(cls, claim: str, text: str) -> AuditStatus:
        """Compare claim text against locator text, fail-closed (D-S2-01).

        A claim with no verifiable terms (or an empty normalized claim) cannot
        be checked and maps to UNKNOWN instead of silently passing.
        """
        ...
        if overlap >= max(1, ceil(len(terms) / 2)):
            return AuditStatus.PASS
    ...
    def _locator_verdict(
        ...
        if not citation.locator:
            ...
            if locator_text is None:
                ...
                match = cls._claim_matches_text(claim.text, locator_text)
                if match == AuditStatus.UNKNOWN:
                    status = AuditStatus.UNKNOWN
                    reason = (
                        "claim has no verifiable terms against the locator text; "
                        "fail-closed to unknown"
                    )
                elif match == AuditStatus.FAIL:
                    status = AuditStatus.FAIL
    ...
    def audit(
        ...
        *,
        ...
        cached = self.store.get("audit_evidence_report", audit_key)
