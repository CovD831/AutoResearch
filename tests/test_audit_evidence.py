...
from autoresearch.audit_evidence import (
    AuditCategory,
    AuditClaim,
    AuditMethod,
    AuditMode,
...
def test_audit_is_idempotent_and_snapshot_version_is_part_of_identity(runtime, project):
    ...
    audit_events = [
        ...
        if event["event_type"] == "audit_evidence.report_created"
    ]
...
def test_audit_persists_reports_and_uses_only_candidate_admission(runtime, project):
    ...
    assert report.admission_results[0].status == EvidenceAdmissionStatus.ACCEPTED
    assert runtime.store.list("evidence", project_id="demo") == []
    assert runtime.store.get("audit_evidence_report", report.audit_key) is not None


def test_vague_claim_locator_is_unknown_not_fail_open(runtime, project):
    """D-S2-01: a claim with no verifiable terms fail-closes to UNKNOWN."""

    service = AuditService(runtime.store, runtime.evidence)
    report = service.audit(
        "demo",
        claims=[_claim(text="To be or not?")],
        citations=[_citation()],
        resolver_snapshot=_resolver(),
        corpus_snapshot=_corpus(),
    )
    locator_verdicts = [
        verdict
        for verdict in report.deterministic_verdicts
        if verdict.category == AuditCategory.LOCATOR_CONSISTENCY
    ]
    assert locator_verdicts
    assert all(verdict.status == AuditStatus.UNKNOWN for verdict in locator_verdicts)
    assert report.status == AuditStatus.UNKNOWN
