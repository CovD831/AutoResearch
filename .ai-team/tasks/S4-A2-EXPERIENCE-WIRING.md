...
- [ ] **失败经验的 `failure` 标签**（规格 `主要交付` 第 2 条原文的一半）：**裁决已定 = (a) 共享 schema**（2026-09-11）。落地分两半 —— owner 应用 `owner-schema.patch`（`contracts.py` / `evolution_service.py`，本包 forbidden），本包应用 `failure-tag-sink.patch`（sink 导出 `FAILURE_TAG` + 2 条测试）。两份补丁一起应用后已实测 **190 passed**（基线 188，+2）/ ruff clean / `experience_sink.py` 200 stmts / 100%，但**尚未应用**，故此项仍未勾选。**顺序不可交换**：只应用 sink 补丁会让两条新测试失败（pydantic `extra="ignore"` 会把 `tags=` 静默丢弃，记录层毫无痕迹），分支会变红。此条不在规格的「验收」四条里，而在「主要交付」里——把它放上验收清单正是为了防止这类「不产生可观测行为的条款」在转述式清单里消失。详见 `L3.md` 的 D-A6-05 落地清单。
...
- **D-A6-05（`failure` 标签 = 共享 schema，已裁决 (a)）**：规格要求把拦截类型模板化为 `ExperienceRecord` 时「打 `failure` 标签」。实测本包无合法通道：`ExperienceRecord`（`contracts.py:367-376`）没有标签字段；唯一标签通道 `WikiPage.tags` 由 `evolution_service.py:27-38` 的 `record()` **硬编码**为 `["experience", grade.value]`；两文件均在本包 `forbidden_paths`。裁决变量不是「标签放哪个字段」而是「`failure` 这个词由谁拥有」：**两个生产者且跨 lane（本包 + B6，见 `P1-B-evidence-lane/TASK-SPECS.md:61`）→ 词表归共享 schema**。故 owner 选 (a)：`ExperienceRecord.tags` + `record()` 透传。落地清单与两份补丁见 `L3.md`；`WikiPage.tags` 参与 `knowledge._score`（tag 2.0/词），所以这不是纯装饰字段。此条是 handoff 后复盘重读任务书时发现的，已在验收清单里落成一条**未勾选**项，不再让它藏在「已完成」的转述里。
...
- D-A6-05 的落地材料（**已验证、未应用**）：`docs/tasks/P1-A-runtime-lane/tasks/A6-experience-wiring/owner-schema.patch`（owner 侧 2 个 forbidden 文件）与 `failure-tag-sink.patch`（本包侧 sink + 2 条测试）。两份一起 `git apply` 后实测 **190 passed**（基线 188，+2）、`ruff check src tests` clean、`experience_sink.py` **200 stmts / 0 miss / 100%**；`git apply --check` 两份都通过。

## Pending

- 规格偏离 D-A6-01 待 owner 裁决。
- **D-A6-05 已裁决 (a)，待执行**：owner 应用 `owner-schema.patch` → 本包应用 `failure-tag-sink.patch` 并把验收项勾上（`8/9` → `9/9`）。两份补丁在原位，顺序不可交换。
...
owner 复核 D-A6-01 裁决、应用 `owner-schema.patch`（D-A6-05 已选 (a)）→ 本包补 `failure-tag-sink.patch` 并复核数字 → 用户侧复跑验收命令与功能场景 → 通过后由 owner 决定 push / 开 PR。下一包 `S4-A3-KNOWLEDGE-VECTOR`。
...
- [x] **D-A6-05 补丁对验证（2026-09-11 22:3x）**：`git apply --check owner-schema.patch` 与 `git apply --check failure-tag-sink.patch` 均通过；两份一起应用后 `.venv/Scripts/python.exe -m pytest tests -o addopts="" -q -p no:cacheprovider --basetemp=...` → **190 passed**（+2），`ruff check src tests` → `All checks passed!`，focused 24 passed，`--cov=autoresearch.experience_sink` → **200 stmts / 100%**。验证后已 `git checkout -- src tests` 回退，本包 diff 仍为零改动于 forbidden 文件。
...
实现完成、已 commit 在 `codex/s4-a2-experience-wiring`，**未 push、未开 PR**。owner 需先复核 D-A6-01（规格写「订阅」，本包按只读拉取落地）、确认 D-A6-02 的派生计数语义，再执行 **D-A6-05 已裁决的 (a)**：应用 `docs/tasks/P1-A-runtime-lane/tasks/A6-experience-wiring/owner-schema.patch`（唯一需要 owner 出手的一步，两个 forbidden 文件、共 2 行）。三个触发通路都已实测可达，不存在 O12 式「接线了但生产上永不触发」。**验收清单为 `8/9`，未勾选那一条就是 `failure` 标签**——本包唯一的未完成项，handoff 后复盘才发现；它的两份补丁已就地、已验证。详见 `docs/tasks/P1-A-runtime-lane/tasks/A6-experience-wiring/HANDOFF.md`。
