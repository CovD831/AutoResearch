...
- [ ] **待 owner 裁决（阻塞 S2-B rebase）**：S2 audit 统一布局与语义——①文件布局（A 保留 `audit.py`/`audit_contracts.py`，B 改名如 `audit_evidence.py` 或合并为单一模块由 A 运行时委托 B 服务）；②record kind/事件 schema（`audit_report` 双 schema 分家或统一 report contract）；③语义分歧收敛：binding（A=引用或有效证据即 PASS/未绑 FAIL vs B=必须有效证据/未绑 UNKNOWN+未验证标记）、corrected 极性（A=FAIL vs B=可追溯关系 PASS）、locator 模型（A=元数据等值 vs B=全文比对）、grade 政策（audit 下限等级）。裁决后记新 D- 决策并同步 A3/B3 任务包。
