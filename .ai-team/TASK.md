...
- [x] 2026-09-11 **A5 / S4 benchmark runtime 交付并合入（runtime 线；owner 集成 PR #19 `8f6e7de`，取代 #18）**：成员 A 交付 PR #18（检索 adapter + benchmark 运行时 + 三组对比）→ owner 深度审查（`reviews/PR18-A5-deep-review.md`）发现**指标语义缺陷族「空/无样本被当作满分」**：`ConditionSummary.score` 在全部单元上取 `accepted_hallucination_ratio` 均值，而未接受单元该值恒为 0 → **拦得越多分越高**（live 实测 `acceptance_rate=0.0` 却 `score=100.0`）；空分母比率报 100.0/0.0；`hall(total_claims=0)=0.0`。**根因再次在规格**（`metric-definition.json` notes 原文即规定「covers all cells and zeroes a non-accepted cell」，同 B5）→ owner 代修 D-A5-12~16：`score` 只对已接受单元求均值、空分母一律 `None`、live 局限声明扩为两条、指标定义升版 1.0→1.1，并补齐 TASK-SPECS 明确要求但未交付的 **F9**（receipt 成本字段位预留——字段逐一对位 O12 类型、默认 `None`=未测得）。**独立盲审复核代修**另抓出 `false_block_rate` 空分母仍报 0.0（与自写 D-A5-13 自相矛盾）。合入后 `main@8f6e7de` 全量 **275 passed**、ruff clean、报告逐位可复现、离线指标数值零变化；新增 10 条测试在原版下全部失败。报告 `reviews/PR15-18-runtime-steelman-review.md`。遗留：F8（三条件从不调 LLM 却名 `bare_llm`）待裁决、`search_service.py` 第二条裸 S2 路径待裁决、`docs/BENCHMARK.md`「四条件」口径差未统一。
- [x] 2026-09-11 **O12 ProviderLane 交付 + owner 代修 C1–C4（Owner 亲自实现；PR #15 open，已交回成员 A 复核）**：段 1（S1.1–S1.5）+ 段 2（S2.0–S2.6）+ S3.1 完成；成员 A 复审提出的 3 条技术意见（默认预算未生效 / 重试未计入预算 / 异常未进统一错误体系）**全部复现为真**，owner 另发现 3 条（`usage` 为 list 时 `AttributeError` 逃逸 / 预算作用域是 per-transport 而非 per-run / 价目跨 provider 顺序查找），已代修 C1–C4（异常面收口 `LaneResponseError`、预置 lane 完整预算 `default_budget_profile`（从 lane 自身价目推导）、预算作用域改 run `LaneRunLedger`、预算按网络请求计）并 rebase 至 `main@1e7e196`（冲突仅记录文件，按并集解决）。产物：`provider_lane.py`（lane 不可变身份/模型目录/四档计价/凭据白名单/漂移守卫/预算档 + httpx 同步 transport 重试分类 + handoff 四规则 + constrained sampling）；`lane_llm_adapter.py`（O12↔B4 薄缝，`is` identity 断言证明契约 diff=0）；`llm.py` 内部改走 lane（公共签名不变、消费方零改动、计价按 model id 解析）；共享 `contracts.py` 增 `TokenUsage`/`InvocationCost`（D-O12-04：`invocation_contracts.py` re-export，`capability_registry.py` 共用）；`data/models_catalog.json` + `data/price_overrides.json` + `PRICE-POLICY.md` + `CONTRACT-PARITY.md`。**评审证据**：全量 `268 passed / 2 skipped / 0 failed`（`-W error`）、ruff 绿、`check.mjs` valid、`check_pr_contract` passed；真实 parity（WorkBuddy 自定义端点）`deepseek-v4-pro` 计价与手算 delta=0、`glm-5.3` 无 catalog 条目记 unpriced（不编造价目）。**审查（`SELF-REVIEW.md`）**：对抗性自查 v2 六项（F1 计价绑 endpoint 字面匹配致静默归零 / F2 空 settings 无 recovery 提示 / F3 空白凭据 fail-late / G1 预算守卫无调用点而文档称已强制 / G2 `provider.lane_drift` 事件缺失 / G3 `LLMService` 未跑漂移校验）+ **独立子代理盲审 v3 四项**（override 零 patch 仍打来源标签 / 负数价目静默入库 / 畸形 override 炸离线目录 / 字符串子串匹配），全部修复。**硬事实**：官方价目实测对照显示本项目快照与 DeepSeek 官方**偏差最高约 4.5×、且缺 peak/off-peak 双档结构** → 计价口径定为 **tokens 为主、cost 为辅**（D-O12-09，带 `price_source`/`attempts`），内置模型按官方 peak 档钉住，A5 任务包同步修订。**待办**：S3.3 成员 A 人工复审（审核不自批，Owner 同为被审方）→ S3.4 registry 回写 `integrated`。诚实注记：`lane_drift_event` 无生产写入路径、peak/off-peak 双档目录结构不可表达、provider 改模型路由刷新脚本发现不了。
