...
# R006-P0's experiment module lives outside the production package on purpose:
# it is a baseline-measurement asset (see its module docstring), not runtime code.
sys.path.insert(0, "experiments/r006-p0")

from experience_injection import (  # noqa: E402
    LiveExperienceAgent,
...
def main() -> int:
    ...
    if not (0.7 <= ratio <= 1.4):
        failures.append(
            f"placeholder/clean prompt length ratio {ratio:.2f} is out of range"
        )
    print(
        f"prompt lengths: off={len_off} clean={len_clean} "
        f"placeholder={len_ph} (ratio {ratio:.2f})"
    )
